...
do
	if [[ -e ${SM}.asn.gz ]]; then
		SZ=`zcat ${SM}.asn.gz | wc -l`
		if [[ ${SZ} -eq 0 ]]; then
			rm ${SM}.fastp.json ${SM}.fastp.html ${SM}.asn.gz
			echo "${SM}.asn.gz empty, remove it and continue" 1>&2
		else
			echo "${SM}	${SM}.asn.gz	${SZ}" >> ${PROJECT}.samples.${SPLIT}
	...
	hmmufotu ${DB} ${P1} ${P2} -o ${SM}.asn.gz --fmt fastq --single FALSE --strand 1 --process 20
	if [[ ${?} -eq 0 ]]; then
		L=`zcat ${SM}.asn.gz | wc -l`
		echo "${SM}	${SM}.asn.gz	$L" >> ${PROJECT}.samples.${SPLIT}
